...
public:
	...
	void set_attribute(int i, const VertexBuffer& vb, ComponentType ct, int cc, bool normalized, int offset, int stride);
...
#define VERTEX_SHADER(A)	"#version 330\n" #A
#define FRAGMENT_SHADER(A)	"#version 330\n" #A
...
public:
	...
	void draw(const RenderState& rs, const Shader& shader, const VertexArray& va);
	void flip_buffers() const;

	Shader::Ptr create_shader(const std::string& vs, const std::string& fs) {
